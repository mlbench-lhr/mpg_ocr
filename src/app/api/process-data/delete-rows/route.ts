...
async function getOracleConnection(user: string, password: string, ip: string, port: string, service: string) {
...
export async function POST(req: Request) {
  try {
    ...
    const connectionStatusRes = await fetch(`${origin}/api/oracle/connection-status`);
...
const fileTable = row?.FILE_TABLE;
  ...
  deleteSummary.push({
    ...
    return NextResponse.json({ error: "Deletion failed", details: String(error) }, { status: 500 });
  }
}
